...
def hash_160_to_bc_address(h160, addrtype = 38):
...
def SecretToASecret(secret, compressed=False, addrtype=38):
    vchIn = chr((addrtype+128)&255) + secret
    if compressed: vchIn += '\01'
    return EncodeBase58Check(vchIn)

def ASecretToSecret(key, addrtype=38):
...
def msg_magic(message):
    ...
    return "\x19Gamecredits Signed Message:\n" + encoded_varint + message
